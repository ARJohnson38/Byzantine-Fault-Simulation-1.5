...
If the video is hosted externally, replace `ADD VIDEO LINK HERE` with an
instructor-accessible share link. If the file is small enough for the
repository, it can instead be stored in the `video/` directory.

The demonstration should show the Docker nodes running, communication
between participating computers, representative Byzantine fault
scenarios, and the resulting decisions.
